...
class TreePE(id: ElemId, n_attr: Int, n_classes: Int, n_depths: Int, info_bit: Int, tree_bit: Int, attr_bit: Int, is_a_root: Boolean, n_loops: Int) extends PE(id){
    ...
    when(RegNext(queue.valid)){
      ...
      if (is_a_root){
        ...
        for(@@)
        io.sample_out.scores := Mux((shift === false.B) && is_valid,scores_bits(offset) + weights_bits(depth_indicator),scores_bits(offset))
        when(shift === false.B){
          when(is_valid){
      ...
      io.sample_out.bits.features := DontCare
      io.sample_out.bits.weights := DontCare
      io.sample_out.bits.tree_to_exec := DontCare
      io.sample_out.bits.scores := DontCare
