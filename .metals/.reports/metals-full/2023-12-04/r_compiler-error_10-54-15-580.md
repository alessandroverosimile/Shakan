    ...
    test(new TreePEsWrapper(n_trees, max_depth, n_attr,n_classes,n_depths,info_bit,tree_bit,attr_bit,bram_size=64*10,structure_list)) { c =>
        ...
        */
        ...
        for (i <- 0 until n_attr){
            val inputBigInt: BigInt = data((i+1)*16-1,i*16).asBigInt
            val fixedPointValue: Double = (inputBigInt >> 8).toDouble + ((inputBigInt & BigInt("FF", 16)).toDouble) / pow(2, 8)
            println(@@)
        }
        /*
        println("SCORES: ")
        for (i <- 0 until n_classes){
          println(c.wrapper_io.sample_out.bits.scores(i).peek().litToDouble)
        }
        println("WEIGHTS: ")
        for (i <- 0 until n_depths){
          println(c.wrapper_io.sample_out.bits.weights(i).peek().litToDouble)
        }
